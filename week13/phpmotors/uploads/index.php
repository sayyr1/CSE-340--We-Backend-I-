...
// Control structure for the server. 
switch ($action) {
    case 'upload':
        ...
        if ($imageCheck) {
            $message = '<p id="alert">An image by that name already exists.</p>';
        } elseif (empty($invId) || empty($imgName)) {
            $message = '<p id="alert">You must select a vehicle and image file for the vehicle.</p>';
        } else {
            ...
            if ($result) {
                $message = '<p id="alert">The upload succeeded.</p>';
            } else {
                $message = '<p id="alert">Sorry, the upload failed.</p>';
            }
        }
    ...
    case 'delete':
        ...
        if ($remove) {
            $message = "<p id='alert'>$filename was successfully deleted.</p>";
        } else {
            $message = "<p id='alert'>$filename was NOT deleted.</p>";
        }
