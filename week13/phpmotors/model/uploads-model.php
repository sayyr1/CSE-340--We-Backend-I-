...
// New function that will obtain thumbnail image information from images
function getThubnailImages($invId){
        $db = phpmotorsConnect();
        $sql = 'SELECT inv.invId, img.imgPath  
        FROM inventory AS inv 
        LEFT JOIN images AS img ON inv.invId = img.invId AND img.imgPath LIKE "%-tn%" WHERE inv.invId = :invId';
        $stmt = $db->prepare($sql);
        $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
        $stmt->execute();
